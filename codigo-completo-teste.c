...
void numeroPorExtenso(int numero)
{
    ...
    if (contador >= 5 && digitos[4]) {
        ...
        if (digitos[4] == 1 && digitos[3] < 10) {
            ...
            else if (digitos[4] >= 2) printf(" Mil");
            else if (digitos[4] == 1 && !digitos[3]) printf(" Mil");
        }
...
int fazerSaque(int saqueCliente, int *estoque_cedulas, int indice_cliente)
{
    if (saqueCliente <= 0 || saqueCliente > 999999) {
        printf("Valor de saque inválido! Deve ser entre 1 e 999999.\n");
        return 0;
    }
...
void valorSaldoExistente()
{
    printf("\n--- Relatório Valor do Saldo Existente na Máquina ---\n");
    ...
    if (saldo_total > 0) {
        ...
        printf("Nenhum saldo disponível na máquina.\n");
    }
}
...
void alterarClientes()
{
    ...
    if (strcmp(resposta, "sim") == 0)
    {
        ...
        do {
            ...
            if (scanf("%d", &resposta_alterar) != 1 || (resposta_alterar != 1 && resposta_alterar != 2)) {
                printf("Opção inválida, digite 1 para CPF ou 2 para Conta Corrente.\n");
                while (getchar() != '\n');
                continue;
            }
            while (getchar() != '\n');
